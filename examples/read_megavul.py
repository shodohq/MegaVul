...
with Path("../megavul/storage/result/c_cpp/megavul_simple.json").open(mode="r") as f:
    ...
    diff_line_info = item[
        "diff_line_info"
    ]  # {'deleted_lines': ['pjmedia_rtcp_comm .... ] , 'added_lines': [ .... ] }
    git_url = item[
        "git_url"
    ]  # https://github.com/pjsip/pjproject/commit/11559e49e65bdf00922ad5ae28913ec6a198d508

    if (
        item["func_graph_path_before"] is not None
    ):  # graphs of some functions cannot be exported successfully
        ...
        print(
            nodes
        )  # [{'version': '0.1', 'language': 'NEWC', '_label': 'META_DATA', 'overlays': ....
        print(
            edges
        )  # [{'innode': 196, 'outnode': 2, 'etype': 'AST', 'variable': None}, ...]
