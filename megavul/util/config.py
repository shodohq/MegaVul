...
SUPPORT_CRAWLING_LANGUAGE = ["c_cpp", "java", "go"]
...
def read_config_file() -> dict:
    ...
    if not config_path.exists():
        raise Exception(
            f"config.yaml config file does not exist, create it in {config_path.absolute()}"
        )

    if not github_token_path.exists():
        raise Exception(
            f"github_token.txt file does not exist, create it in {github_token_path.absolute()}"
        )
    ...
    for token in github_token_path.open(mode="r").readlines():
        if token.startswith("ghp") or token.startswith("gho"):
            ...
            raise Exception(
                f'Invalid github token {token}, must start with "ghp" or "gho"'
            )
