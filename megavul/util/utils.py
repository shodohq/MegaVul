...
if "dependencies" not in config_file:
    raise RuntimeError(
        "Dependencies [java, scala, sbt, node, tree-sitter, github-linguist] missing"
    )
else:
...
def read_urls_from_file(path: Path) -> list[str]:
    return [li.rstrip() for li in path.open(mode="r").readlines()]
...
def load_from_marshmallow_dataclass_json_file(
    clazz: type, fp: Path, is_list=True
) -> list | dict:
    return class_schema(clazz)().loads(fp.open(mode="r").read(), many=is_list)
...
def build_tree_sitter_language(language_name: str, debug_mode=False) -> Language:
    ...
    tree_sitter_so = (
        StorageLocation.result_dir() / "build" / f"build-{tree_sitter_name}.so"
    )
...
def compress_directory_to_zip(compress_directory: Path, output_zip_path: Path):
    ...
    """
    shutil.make_archive(str(output_zip_path.with_suffix("")), "zip", compress_directory)
...
if __name__ == "__main__":
    ...
    print(
        convert_to_jvm_proxy(
            {"http": "http://www.google.com", "https": "http://www.google.com:1234"}
        )
    )
