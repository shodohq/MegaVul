...
def child_process_wrapper(
    queue: multiprocessing.Queue,
    inner_func: Callable[[logging.Logger, _T], _S]
    | Callable[[logging.Logger, List[_T]], List[_S]],
    chunk: List[_T],
    chunk_mode: bool,
):
...
def multiprocessing_apply_data_with_logger(
    func_with_logger: Callable[[logging.Logger, _T], _S]
    | Callable[[logging.Logger, List[_R]], List[_S]],
    data: List[_T | _R],
    chunk_mode=False,
    debug=False,
) -> List[_S]:
    if debug:
        global_logger.debug(
            "Running multiprocessing_apply_data_with_logger in DEBUG mode will run in single process"
        )
        res = []
        if chunk_mode:
    ...
    global_logger.info(
        f"Multiprocessing begin process {len(data)}(chuck size {chunk_size}) items in {max_processors} processors"
    )
    ...
    with multiprocessing.Manager() as sync_manager:
        ...
        with multiprocessing.Pool() as pool:
            ...
            results = [
                pool.apply_async(
                    child_process_wrapper,
                    args=(queue, func_with_logger, chunks[i], chunk_mode),
...
def wrapper_func(
    x: _T, func: Callable[[logging.Logger, _T], _S], logger: logging.Logger
) -> _S:
