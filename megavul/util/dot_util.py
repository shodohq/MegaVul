    ...
    def open_in_browser(self):
        ...
        with tempfile.NamedTemporaryFile(
            ...
            with tempfile.NamedTemporaryFile(
                ...
                _p = subprocess.run(
                    f"dot -Tsvg {dot_tmp_file_path} -o {svg_tmp_file_path}",
                    shell=True,
                    check=True,
                )
...
class JoernGraphGenerator:
    ...
    def __filter_edge(self, edges: list, edge_type: str | None):
        assert edge_type in _JOERN_EDGE_LABEL_TYPES, (
            f"Edge type {edge_type} is not supported, support edge type: {_JOERN_EDGE_LABEL_TYPES}"
        )
    ...
    def create_dot_graph(self, edge_type: str | None = None) -> DotGraphGenerator:
        ...
        appear_nodes = set(
            [
                item
                for sub_tuple in [(e["inNode"], e["outNode"]) for e in edges]
