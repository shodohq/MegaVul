...
class MegaVulGraphGenerateForJavaScriptTest extends JsSrc2CpgSuite {

  def importCode(inputFilePath: String): Cpg = {
    val content = Files.readString(Paths.get(inputFilePath))
    if (inputFilePath.endsWith(".js") || inputFilePath.endsWith(".mjs") ||
        inputFilePath.endsWith(".cjs") || inputFilePath.endsWith(".jsx")) {
      return code(content)
    }
    else
      throw new Exception(s"${inputFilePath} suffix can't recognized, only support .js/.mjs/.cjs/.jsx suffix")
  }
  ...
  def generateGraphForFunction(function_dir_path: Path): Unit = {
    ...
    val func_path = function_dir_path.resolve(s"${file_name}.js")
  ...
  "generateGraph" should {
    ...
    println("MegaVul Generate JavaScript Graph Done!")
  }
