...
class MegaVulGraphGenerateForPythonTest extends PySrc2CpgFixture {

  def importCode(inputFilePath: String): Cpg = {
    val content = Files.readString(Paths.get(inputFilePath))
    if (inputFilePath.endsWith(".py")) {
      new PySrcTestCpg().moreCode(content, "main.py")
    }
    else
      throw new Exception(s"${inputFilePath} suffix can't recognized, only support .py suffix")
  }
  ...
  def generateGraphForFunction(function_dir_path: Path): Unit = {
    ...
    val func_path = function_dir_path.resolve(s"${file_name}.py")
  ...
  "generateGraph" should {
    ...
    println("MegaVul Generate Python Graph Done!")
  }
