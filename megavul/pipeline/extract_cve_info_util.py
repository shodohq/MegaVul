...
def find_xen_commit_from_advisory(advisory_url: str) -> list[str]:
    ...
    match_re = (
        rf"this is (cve[^\s]* / )?{xsa_id}|this is part of (cve[^\s]* / )?{xsa_id}"
    )
    return find_commit_from_commit_msg_in_github(
        "xen-project/xen", xsa_id, regex_match=match_re
    )
...
def find_commit_from_chromium_code_review(review_url: str) -> Optional[str]:
    ...
    assert hostname and (hostname == "codereview.chromium.org" or hostname.endswith(".codereview.chromium.org"))
...
def find_chromium_commit_from_viewvc(viewvc_url: str) -> Optional[str]:
    ...
    if chromium_log_msg2hash is None:
        chromium_log = (StorageLocation.storage_dir() / "chromium_log.txt").open(
            mode="r"
        )
        chromium_log_lines = chromium_log.readlines()
        chromium_log_msg2hash = {}
        for lo in chromium_log_lines:
            hash, msg = lo.split("||||||||||")
            msg = msg.strip()
...
def find_commit_from_php_issue(php_issue_url: str) -> list[str]:
    ...
    commit_urls = list(
        map(lambda x: f"https://github.com/php/php-src/commit/{x}", commit_hash_set)
    )
    return commit_urls
...
def find_commit_from_gnome_bugzilla(gnome_url: str) -> list[str]:
    ...
    status = " ".join(
        [
            s.strip()
            for s in page.find("td", id="bz_field_status").text.strip().split("\n")
        ]
    )
    if status != "RESOLVED FIXED":  # this issue has not been resolved
        return []
...
def find_commit_from_ghostscript_bugzilla(ghost_url: str) -> list[str]:
    ...
    for c in comments:
        ...
        commit_pattern = (
            comment_text.find("author:") != -1 and comment_text.find("date:") != -1
        )
        fix_in_pattern = (
            comment_text.find("fixed in") != -1 or comment_text.find("fix in") != -1
        )
        # fix in pattern
        # ###################################################################
