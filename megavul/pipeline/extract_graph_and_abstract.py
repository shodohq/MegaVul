...
from megavul.util.concurrent_util import (
...
generate_source_dir = (
    StorageLocation.cache_dir() / crawling_language / "joern_file_cache"
)
...
def generate_source_file(
    ...
    save_index_file = save_dir / "MegaVul_index.json"
...
def run_joern_once(timeout) -> int:
    ...
    try:
        ...
        global_logger.info("Joern run time out, terminating the process group...")
...
def run_joern():
    ...
    while True:
        ...
        if return_code > 0:
            ...
            global_logger.info("[Joern] run error happened")
            break
...
def call_joern_to_generate_graph():
    ...
    if crawling_language == CrawlingType.C_CPP:
        joern_script_path = (
            ...
            / "MegaVulGraphGenerateForCAndCppTest.scala"
        )
    elif crawling_language == CrawlingType.Java:
        joern_script_path = (
    ...
    if not joern_path.exists():
        global_logger.error(
            f"Missing joern source code in {joern_path}, please git clone joern first."
        )
        return
    ...
    if crawling_language == CrawlingType.C_CPP:
        shutil.copy(
            joern_script_path,
            joern_path
            / "joern-cli/frontends/c2cpg/src/test/scala/io/joern/c2cpg/io"
            / joern_script_name,
        )
    elif crawling_language == CrawlingType.Java:
        shutil.copy(
        ...
        go_test_dir = (
            joern_path
            / "joern-cli/frontends/gosrc2cpg/src/test/scala/io/joern/go2cpg/io"
        )
    ...
    global_logger.info("Checking all joern generated json files")
...
def find_successfully_extracted_func_graph(
    logger: logging.Logger, cve: CveWithCommitInfo
):
    save_dir = generate_source_dir
    ...
    for commit in cve.commits:
        ...
        for file in commit.files:
            this_file_dir = this_commit_dir / file.file_name
            this_file_graph_save_dir = (
                graph_save_dir / repo_name / commit_hash / file.file_name
            )
            # file_language = file.language
            ...
            for idx, vul_func in enumerate(file.vulnerable_functions):
                ...
                if before_complete and after_complete:
                    # all correct
                    final_before_graph_path = (
                        this_file_graph_save_dir / "vul" / "before" / f"{idx}.json"
                    )
                    final_before_graph_path.parent.mkdir(parents=True, exist_ok=True)
                    final_after_graph_path = (
                        this_file_graph_save_dir / "vul" / "after" / f"{idx}.json"
                    )
                    ...
                    # populate field
                    vul_func.func_graph_path_before = str(
                        final_before_graph_path.relative_to(graph_save_dir)
                    )
                    vul_func.func_graph_path_after = str(
                        final_after_graph_path.relative_to(graph_save_dir)
                    )
            ...
            for idx, non_vul_func in enumerate(file.non_vulnerable_functions):
                ...
                if check_func_graph_complete(logger, graph_path):
                    final_graph_path = (
                        this_file_graph_save_dir / "non_vul" / f"{idx}.json"
                    )
                    ...
                    non_vul_func.func_graph_path = str(
                        final_graph_path.relative_to(graph_save_dir)
                    )
...
def abstracting_functions(logger: logging.Logger, cve: CveWithCommitInfo):
    ...
    for commit in cve.commits:
        for file in commit.files:
            ...
            vul_func: VulnerableFunction
            for idx, vul_func in enumerate(file.vulnerable_functions):
                abstract_func_before, symbol_table_before = (
                    code_abstracter.abstract_code(vul_func.func_before, language)
                )
                abstract_func_after, symbol_table_after = code_abstracter.abstract_code(
                    vul_func.func_after, language
                )
            ...
            non_vul_func: NonVulnerableFunction
            for idx, non_vul_func in enumerate(file.non_vulnerable_functions):
                abstract_func, symbol_table = code_abstracter.abstract_code(
                    non_vul_func.func, language
                )
    ...
    for cve in cve_list:
        for commit in cve.commits:
            for file in commit.files:
                ...
                parse_succeed_vul += sum(
                    [
                        f.func_graph_path_before is not None
                        for f in file.vulnerable_functions
                    ]
                )
                parse_succeed_non_vul += sum(
                    [
                        f.func_graph_path is not None
                        for f in file.non_vulnerable_functions
                    ]
                )

    global_logger.info(
        f"Find successfully parsed "
...
def extract_graph_and_abstract():
    ...
    cve_with_commit = load_from_marshmallow_dataclass_json_file(
        CveWithCommitInfo, cve_with_parsed_and_filtered_commit_json_path, True
    )
