...
def crawl_nvd(use_cache: bool = True):
    ...
    global_logger.info(
        f"Begin crawl CVE entries from NVD database, total entries:{nvd_metadata.totalResults}, total pages:{total_page_cnt}"
    )
    all_cve_entries = []
    ...
    if result_save_path.exists():
        ...
        if old_result_entries_cnt == nvd_metadata.totalResults and use_cache:
            ...
            global_logger.info(
                f"Out-of-date data, old:{old_result_entries_cnt} now:{nvd_metadata.totalResults}, try to download the latest NVD database"
            )
            # remove old cache
