...
from megavul.pipeline.json_save_location import (
    cve_with_reference_url_json_path,
...
from megavul.pipeline.extract_cve_info_util import (
    find_sourceware_commit_from_comment,
    find_xen_commit_from_advisory,
    find_commit_from_chromium_code_review,
    find_chromium_commit_from_viewvc,
    find_url_from_debian_security_tracker,
    find_commit_from_moodle_discuss,
    find_commit_from_php_issue,
    find_commit_from_rustsec,
    find_commit_from_gnome_bugzilla,
    find_commit_from_ghostscript_bugzilla,
)
    ...
    for url in urls:
        ...
        if nloc == "github.com":
            ...
            url = (
                ...
                .replace("a=commit_plain", "a=commit")
                .replace("patch", "commit")
            )
            if "commit" in url:
                url_result.append(url)
            ...
            if "commit" in url:
                ...
                url_result.extend(
                    [gitlab_url_to_github_url(url) for url in gitlab_urls]
                )
            ...
            if "/commit/" in url:
                ...
                not url.startswith("https://gitlab.com/gitlab-org")
            ):  # Do NOT track gitlab-org repo
                gitlab_urls = find_commits_from_gitlab(url)
            ...
            extracted_urls = mining_commit_urls_from_reference_urls(
                logger, potential_urls
            )
            ...
            if "issue" in url:
                commit_urls = mining_commit_urls_from_reference_urls(
                    logger, [get_final_redirect_url(url)]
                )
...
def dump_url():
    ...
    global_logger.info(
        f"Reference urls from {len(website_url_dict)} different websites"
    )

    url_filter_by_frequency = {
        k: v
        for k, v in sorted(
            filter(
                lambda item: len(item[1]) > DUMP_URL_FREQUENCY, website_url_dict.items()
            ),
    ...
    def add_to_all_in_one(netloc_name: str, netloc_urls: list):
        ...
        latest_urls = list(
            OrderedSet(netloc_urls)[-DUMP_URL_FREQUENCY:]
        )  # save latest urls
        all_in_one_res += "\n".join(latest_urls)
...
def parse_single_cve(
    logger: logging.Logger, cve_row: dict
) -> Optional[CveWithReferenceUrl]:
    # useful field:
    ...
    if (cve_row["vulnStatus"] not in ["Analyzed", "Modified"]) or (
        "weaknesses" not in cve_row
    ):
        return
    ...
    extracted_reference_url = filter_duplicate(
        mining_commit_urls_from_reference_urls(logger, references)
    )
    if DUMP_URL_FLAG:
    ...
    return CveWithReferenceUrl(
        ...
        publish_date,
        last_modify_date,
        extracted_reference_url,
        cvss_metric.cvss_vector,
        cvss_metric.cvss_score,
        cvss_metric.cvss_severity,
        cvss_metric.cvss_3,
    )
...
def extract_cve_info():
    ...
    global_logger.info(
        f"Extracting cve info, reference urls and find potential commits from {len(nvd_cve_entries)} CVE entries"
    )
    cve_with_reference_urls: list[Optional[CveWithReferenceUrl]] = (
        multiprocessing_apply_data_with_logger(
            parse_single_cve, nvd_cve_entries, debug=DUMP_URL_FLAG
        )
    )
    global_logger.info("Extracting cve info done!")
    cve_with_reference_urls: list[CveWithReferenceUrl] = list(
        filter(
            lambda x: x is not None and len(x.reference_urls) != 0,
            cve_with_reference_urls,
        )
    )
    global_logger.info(
    ...
    save_marshmallow_dataclass_to_json_file(
        CveWithReferenceUrl, cve_with_reference_url_json_path, cve_with_reference_urls
    )
