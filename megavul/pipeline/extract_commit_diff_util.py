...
def traverse_all_commit(
    cve_with_commit: list[CveWithDownloadedCommitInfo],
) -> Iterator[DownloadedCommitInfo]:
    for cve in cve_with_commit:
        for commit in cve.commits:
            yield commit


def traverse_single_commit(
    commit: DownloadedCommitInfo,
...
class ExtractCommitDiffRecorder:
    ...
    def record_and_print_raw_info(
        self, logger: logging.Logger, cve_with_commit: list[CveWithDownloadedCommitInfo]
    ):
    ...
    def record_and_print_diff_info(
        self, logger: logging.Logger, cve_with_parsed_commit: list[CveWithCommitInfo]
    ):
