...
from megavul.pipeline.extract_commit_diff_filter import (
    ...
    TestFunctionFilter,
    TestFileFilter,
    MultiCveCommitFilter,
    OneCveMultipleCommitsNonVulDuplicateFilter,
    OneCveMultipleCommitsByContentDuplicateFilter,
    LargeChangeFilter,
    LargeNonVulFunctionFilter,
    LargeVulFunctionFilter,
    RemoveNonVulAppearInVulFilter,
    RemoveNonVulDuplicateFilter,
)
from megavul.pipeline.extract_commit_diff_statistic import MetricsGlobalFilter
from megavul.pipeline.extract_commit_diff_util import (
    traverse_all_commit,
    RepoType,
    traverse_single_commit,
    get_file_type_using_linguist,
    ExtractCommitDiffRecorder,
    difflib_diff_func,
)
from megavul.pipeline.json_save_location import (
    cve_with_parsed_commit_json_path,
    cve_with_downloaded_commit_json_path,
...
from megavul.util.utils import (
    load_from_marshmallow_dataclass_json_file,
...
parsed_commit_cache_dir = (
    StorageLocation.cache_dir() / crawling_language / "commit_file_parsed_cache"
)
commit_file_cache_dir = (
    StorageLocation.cache_dir() / crawling_language / "commit_file_cache"
)
...
def get_file_type(repo_type: RepoType, f_name: str, fp: Path) -> str:
    ...
    if crawling_language == CrawlingType.C_CPP:
        if (f_ext in CFileExtension) or (
            f_ext in HeaderExtension and repo_type == RepoType.PureC
        ):
            return "c"
        elif (f_ext in CppFileExtension) or (
            f_ext in HeaderExtension and repo_type == RepoType.PureCpp
        ):
            return "cpp"
...
def parse_all_commit_files(
    logger: logging.Logger,
    cve_with_commit: list[CveWithDownloadedCommitInfo],
    all_repo_type_mapping: dict[str, RepoType],
) -> None:
    if crawling_language == CrawlingType.C_CPP:
    ...
    for commit in traverse_all_commit(cve_with_commit):
        ...
        for base_hash, hash, f_path in traverse_single_commit(commit):
            ...
            raw_file = (
                commit_file_cache_dir / commit.repo_name / base_hash / hash / f_name
            )

            # save destination
            parsed_file_save_dst = (
                parsed_commit_cache_dir / new_repo_name / base_hash / hash / f_name
            )
...
def check_commit_all_file_parsed_successfully(
    logger: logging.Logger, commit: DownloadedCommitInfo
) -> bool:
    ...
    for f_path in commit.diff_file_paths:
        ...
        if not check_file_exists_and_not_empty(this_commit_dir / f_name):
            logger.info(
                f"{commit.repo_name}:{commit.commit_hash} {f_name} not successfully parsed,"
        ...
        if not check_file_exists_and_not_empty(parent_commit_dir / f_name):
            logger.info(
                f"{commit.repo_name}:{commit.parent_commit_hash} {f_name} not successfully parsed,"
...
def get_file_functions_name_mapping(
...
def find_successfully_parsed_commit(
    logger: logging.Logger,
    cve_with_commit: list[CveWithDownloadedCommitInfo],
    all_repo_type_mapping: dict[str, RepoType],
    ...
    for cve in cve_with_commit:
        ...
        for commit in cve.commits:
            parse_successfully = check_commit_all_file_parsed_successfully(
                logger, commit
            )
            ...
            for f_path in commit.diff_file_paths:
                ...
                this_file_funcs = load_from_marshmallow_dataclass_json_file(
                    ExtractedFunction, this_file_path, is_list=True
                )
                parent_file_funcs = load_from_marshmallow_dataclass_json_file(
                    ExtractedFunction, parent_file_path, is_list=True
                )

                this_file_func_name_mapping = get_file_functions_name_mapping(
                    this_file_funcs
                )
                parent_file_func_name_mapping = get_file_functions_name_mapping(
                    parent_file_funcs
                )

                func_name_intersect = list(
                    this_file_func_name_mapping.keys()
                    & parent_file_func_name_mapping.keys()
                )
                ...
                for func_name in func_name_intersect:
                    ...
                    if this_func.func == parent_func.func:
                        ...
                        non_vulnerable_funcs.append(
                            NonVulnerableFunction(
                                this_func.func_name,
                                this_func.parameter_list_signature,
                                this_func.parameter_list,
                        ...
                        diff_func, diff_line_dict = difflib_diff_func(
                            parent_func.func, this_func.func
                        )

                        vulnerable_funcs.append(
                            VulnerableFunction(
                                parent_func.func_name,
                                parent_func.parameter_list_signature,
                                parent_func.parameter_list,
        ...
        if len(commit_infos) > 0:
            ...
            new_cve = CveWithCommitInfo(
                **cve_dict, commits=commit_infos
            )  # update commits
            result_cve.append(new_cve)
...
def extract_successful_parsed_commit(
    ...
    if cve_with_parsed_commit_json_path.exists() and using_cache:
        ...
        return load_from_marshmallow_dataclass_json_file(
            CveWithCommitInfo, cve_with_parsed_commit_json_path
        )
    ...
    if not cve_with_downloaded_commit_json_path.exists():
        raise RuntimeError(
            f"{cve_with_downloaded_commit_json_path} not found, run extract_and_download_commit.py to download commits first"
        )
    ...
    cve_with_parsed_commit: list[CveWithCommitInfo] = (
        multiprocessing_apply_data_with_logger(
            partial(
                find_successfully_parsed_commit,
                all_repo_type_mapping=all_repo_type_mapping,
            ),
            cve_with_commit,
            chunk_mode=True,
        )
...
def run_filters_with_parsed_commit(cve_with_parsed_commit: list[CveWithCommitInfo]):
    ...
    global_logger.info(
        f"All filters have been run, save result to {cve_with_parsed_and_filtered_commit_json_path}"
    )
    save_marshmallow_dataclass_to_json_file(
...
def only_run_statistical_filter():
    ...
    run_filters(
        cve_with_parsed_and_filtered_commit,
        [
