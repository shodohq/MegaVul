...
from megavul.pipeline.extract_commit_diff_filter import GlobalFilter
...
def tree_map():
    d = {
        ...
        "openssl": 125,
        "qemu": 113,
        "radareorg/radare2": 105,
        "the-tcpdump-group/tcpdump": 96,
        "FFmpeg": 95,
        "ArtifexSoftware/ghostpdl": 73,
        "freetype/freetype2": 49,
        "jerryscript-project/jerryscript": 45,
    }
    data = {
...
class MetricsGlobalFilter(GlobalFilter):
    ...
    def get_monthly_cve_cnt(self, cve_cnt: dict) -> tuple[list, list]:
        ...
        month_cve_cnt = dict(
            sorted(cve_cnt.items(), key=lambda x: concat_year_month(x[0]), reverse=True)
        )
    ...
    def get_yearly_cve_cnt(self, cve_cnt: dict):
        ...
        for k, v in cve_cnt.items():
            ...
            new_cve_cnt[year] += v
        year_cve_cnt = dict(
            sorted(new_cve_cnt.items(), key=lambda x: int(x[0]), reverse=True)
        )
    ...
    def year_and_month_cve_report(self, cve_cnt: dict):
        ...
        self.simple_chart_plot(
            axes[1], year_cve_cnt[0], year_cve_cnt[1], "Yearly CVE Report"
        )
        fig.tight_layout()
    ...
    def get_sorted_repo_metric(
        self,
        repo_statistics: dict[str, RepositoryMetrics],
        get_field: Callable[[RepositoryMetrics], int],
    ...
    def repo_commits_cnt(
        ...
        # get top-20 result
        self.simple_chart_plot(
            ax, repo_names[:20], repo_commits[:20], "Repository Commit"
        )
    ...
    def repo_vul_func_cnt(
        ...
        repo_names, func_cnt = self.get_sorted_repo_metric(
            repo_statistics, lambda x: x.vul_cnt if not is_non_vul else x.non_vul_cnt
        )
        self.simple_chart_plot(
            ax,
            repo_names[:15],
    ...
    def repo_cve_cnt(
        ...
        self.simple_chart_plot(
            ax, repo_names[:20], cve_cnt[:20], "Repository Related CVE"
        )
    ...
    def cve_year_repo_cnt_plot(
        self, repo_statistics: dict[str, RepositoryMetrics], ax: Axes
    ):
        top_repo_names, _ = self.get_sorted_repo_metric(
            repo_statistics, lambda x: x.cve_cnt
        )
        ...
        possible_years = list(
            sorted(possible_years, key=lambda x: int(x), reverse=True)
        )
        ...
        bar_heights = {
            label: [
                cve_year_repo_cnt[year][label]
                if label in cve_year_repo_cnt[year]
                else 0
                for year in years
            ]
            for label in top_repo_names
        }
        bottom = [0] * len(years)
    ...
    def get_top_cwe_and_plot(
        ...
        self.simple_chart_plot(
            ax,
            top_cwe_names,
            list(sorted_cwe_cnt.values())[:TOP_CWE],
            f"Top {TOP_CWE} CWE",
        )
        return top_cwe_names

    def cve_year_cwe_cnt_plot(
        self,
        repo_statistics: dict[str, RepositoryMetrics],
        sorted_cwe_names: list[str],
        ax: Axes,
    ):
        cve_year_cwe_cnt = {}
        ...
        bar_heights = {
            label: [
                cve_year_cwe_cnt[year][label] if label in cve_year_cwe_cnt[year] else 0
                for year in years
            ]
            for label in top_10_cwe_names
        }
        bottom = [0] * len(years)
    ...
    def repo_cwe_cnt_plot(
        ...
        bar_heights = {
            cwe_name: [
                repo_cwe_cnt[repo][cwe_name] if cwe_name in repo_cwe_cnt[repo] else 0
        ...
        bar_bottoms = {
            cwe_name: [
                repo_bottoms[repo][cwe_name] if cwe_name in repo_bottoms[repo] else 0
                for repo in top_repo_names
            ]
            for cwe_name in sorted_cwe_names
        }
        ...
        for idx, label in enumerate(bar_heights):
            ...
            ax.bar(
                ...
                bar_heights[label],
                label=label,
                bottom=bar_bottoms[label],
                color=color,
            )

        ax.set_xticklabels(
            top_repo_names, rotation=30, ha="right", rotation_mode="anchor", fontsize=7
        )
        ax.legend()

    def cwe_repo_cnt_plot(
        self,
        repo_statistics: dict[str, RepositoryMetrics],
        sorted_cwe_names: list[str],
        ax: Axes,
        ...
        for cwe_id in sorted_cwe_names:
            ...
            sorted_repo_cnt = sorted(
                repo_cnt_dict.items(), key=lambda x: x[1], reverse=True
            )
    ...
    def filter(self, cve_list: list[CveWithCommitInfo]) -> list[CveWithCommitInfo]:
        ...
        for cve in cve_list:
            ...
            for commit in cve.commits:
                ...
                repo_statistics.setdefault(
                    repo_name, RepositoryMetrics(repo_name, 0, 0, 0, 0, [], [])
                )
                ...
                for file in commit.files:
                    ...
                    for v in file.vulnerable_functions:
                        commit_diff_cnt += len(v.diff_line_info["deleted_lines"]) + len(
                            v.diff_line_info["added_lines"]
                        )
                ...
                old_metrics.commits.append(
                    CommitMetrics(
                        ...
                        commit.git_url,
                        len(commit.files),
                        vul_func_cnt_in_commit,
                        non_vul_func_cnt_in_commit,
                    )
                )
                if (
                    len(old_metrics.cves) > 0
                    and old_metrics.cves[-1].cve_id == cve.cve_id
                ):  # duplicate add
                    pass
                else:
                    old_metrics.cve_cnt += 1
                    old_metrics.cves.append(
                        SimpleCve(cve.cve_id, cve.publish_date, cve.cwe_ids)
                    )
        ...
        boxplot(
            non_vul_line_per_file_cnt, "non_vulnerable_function_per_file_lines_boxplot"
        )
