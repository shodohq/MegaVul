...
def add_vul_func(
    dst: list[MegaVulFunction],
    simple_dst: list[MegaVulSimpleFunction],
    cve: CveWithCommitInfo,
    commit: CommitInfo,
    file_path: str,
    ...
    dst.append(
        MegaVulFunction(
            ...
            file_path,
            vul_func.func_name,
            vul_func.parameter_list_signature_before,
            vul_func.parameter_list_before,
            vul_func.return_type_before,
            vul_func.func_before,
            vul_func.abstract_func_before,
            vul_func.abstract_symbol_table_before,
            vul_func.func_graph_path_before,
            vul_func.parameter_list_signature_after,
            vul_func.parameter_list_after,
            vul_func.return_type_after,
            vul_func.func_after,
            vul_func.abstract_func_after,
            vul_func.abstract_symbol_table_after,
            vul_func.func_graph_path_after,
            vul_func.diff_func,
            vul_func.diff_line_info,
            True,
        )
    )

    simple_dst.append(
        MegaVulSimpleFunction(
            cve.cve_id,
            cve.cwe_ids,
            cve.cvss_vector,
            cve.cvss_is_v3,
...
def add_non_vul_func(
    ...
    dst.append(
        MegaVulFunction(
            ...
            None,
            None,
            non_vul_func.parameter_list_signature,
            non_vul_func.parameter_list,
            non_vul_func.return_type,
            non_vul_func.func,
            non_vul_func.abstract_func,
            non_vul_func.abstract_symbol_table,
            non_vul_func.func_graph_path,
            None,
            None,
            False,
        )
...
def generate_megavul():
    ...
    """
    cve_with_graph_abstract_commit: list[CveWithCommitInfo] = (
        load_from_marshmallow_dataclass_json_file(
            CveWithCommitInfo, cve_with_graph_abstract_commit_json_path, True
        )
    )
        ...
        for commit in cve.commits:
            for file in commit.files:
                ...
                for f in file.vulnerable_functions:
                    add_vul_func(
                        final_result, simple_final_result, cve, commit, file_path, f
                    )
    ...
    save_marshmallow_dataclass_to_json_file(
        MegaVulFunction, megavul_json_path, final_result
    )
    save_marshmallow_dataclass_to_json_file(
        MegaVulSimpleFunction, megavul_simple_json_path, final_result
    )
