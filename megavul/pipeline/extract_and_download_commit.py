...
def run_git_platform_pipeline(
    logger: logging.Logger, cve_with_reference_url: list[CveWithReferenceUrl]
) -> list[CveWithDownloadedCommitInfo]:
    process_name = current_process().name
    logger.info(
        f"{process_name} begin process {len(cve_with_reference_url)} CVE entries"
    )
    ...
    for idx, cve in enumerate(cve_with_reference_url):
        if idx % report_progress_step == 0 and idx > 0:
            logger.info(
                f"{process_name} processed [{idx}/{len(cve_with_reference_url)}] CVE entries"
            )
        ...
        for ref_url in cve.reference_urls:
            ...
            for git_pf in git_pfs:
                if git_pf.can_handle_this_url(logger, ref_url, ref_url_netloc):
                    ...
                    resolve_commit = git_pf.resolve_raw_commit_and_download(
                        logger, raw_commit
                    )
                    ...
                    for pre_commit in cve_resolve_commits:
                        ...
                        this_commit_repo_name = try_repo_name_merge(
                            resolve_commit.repo_name
                        )
...
def extract_and_download_commit():
    cve_with_reference_url: list[CveWithReferenceUrl] = (
        load_from_marshmallow_dataclass_json_file(
    ...
    global_logger.info(
        f"Begin download commits from commit url, total CVE entries: {len(cve_with_reference_url)}"
    )
    commit_website_statistic(cve_with_reference_url)

    downloaded_commit_cves = multiprocessing_apply_data_with_logger(
        run_git_platform_pipeline, cve_with_reference_url, chunk_mode=True
    )

    global_logger.info(
        f"Successfully download commits from {len(downloaded_commit_cves)} CVE entries"
    )
    global_logger.info(
        f"Save CveWithDownloadedCommitInfo to {cve_with_downloaded_commit_json_path}"
    )
    save_marshmallow_dataclass_to_json_file(
        CveWithDownloadedCommitInfo,
        cve_with_downloaded_commit_json_path,
