...
class ParserCLike(ParserBase, metaclass=ABCMeta):
    def parse_file(self, fp: Path, result_save_path: Path):
        ...
        remove_comment_src = ParserCLike.replace_comments_with_whitespace(
            "".join(file_lines)
        )
        try:
            ...
            self.logger.debug(
                f"remove preprocessor failed for {fp} , rollback to raw file"
            )
        ...
        try:
            ...
            save_marshmallow_dataclass_to_json_file(
                ExtractedFunction, result_save_path, extracted_funcs
            )
    ...
    def remove_preprocessor(code: str) -> str:
        ...
        for line in code:
            ...
            if re_res := re.match(
                r"^#[ \t]*(?P<macro>ifdef|ifndef|else|elif|endif|if)", strip_line
            ):
                ...
                if macro in ["ifdef", "ifndef", "if"]:
                    ...
                    if (
                        len(encounter_else_stack) > 0
                        and encounter_else_stack[-1] != nested_level
                    ) or len(encounter_else_stack) == 0:
                    ...
                    if (
                        len(encounter_else_stack) > 0
                        and encounter_else_stack[-1] == previous_level
                    ):
        ...
        for line in code:
            strip_line = line.strip()
            if re_res := re.match(
                r"^#[ \t]*(ifdef|ifndef|else|elif|endif|if)", strip_line
            ):
    ...
    def multiline_replace(lines: list[str], start_point: tuple, replace_id: str):
        ...
        modify_line = modify_line[: start_point[1]] + modify_line[
            start_point[1] :
        ].replace(replace_id, ParserCLike.SPECIAL_IDENTIFIER, 1)  # replace one time
    ...
    def replace_comments_with_whitespace(csrc: str) -> str:
        ...
        csrc = re.sub(
            regex, replace_block_comment, csrc, flags=re.MULTILINE | re.DOTALL
        )
