...
class ParserJava(ParserBase):
    ...
    def traverse_method_declaration(self, func_node: Node, file_lines: list[str]):
        ...
        while True:
            if parameter_list_cursor.node.type == "formal_parameter":
                ...
                parameter_list.append(
                    self.traverse_spread_parameter_declaration(
                        parameter_list_cursor.node, file_lines
                    )
                )
        ...
        return ExtractedFunction(
            method_name, compose_signature, parameter_list, return_type, func
        )

    def traverse_parameter_declaration(
        self, node: Node, file_lines: list[str]
    ) -> Tuple[str, str, int]:
    ...
    def traverse_spread_parameter_declaration(
        self, node: Node, file_lines: list[str]
    ) -> Tuple[str, str, int]:
        cursor = node.walk()
