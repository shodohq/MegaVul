...
class CodeAbstracterBase(metaclass=ABCMeta):
    ...
    def abstract_code(self, code: str, language: str) -> tuple[str, dict]:
        ...
        position_map: dict[
            str, list[tuple]
        ] = {}  # row -> list[(col, internal_id, abstract_type, text, need_replace_to_whitespace)]
        ...
        def add_to_position_map(
            ...
            position_map[str(row)].append(
                (col, internal_id, abstract_type, node_text, need_replace_to_whitespace)
            )
            ...
            if len(node_text.splitlines()) > 1:  # multiline lines string literal
                ...
                for row in range(start_row, start_row + len(node_texts)):  # row number
                    ...
                    if row == start_row:
                        ...
                        add_to_position_map(
                            row, col, global_id, abstract_type, node_text, False
                        )
                    else:
                        # rest of lines
                        add_to_position_map(
                            row, col, global_id, abstract_type, node_text, True
                        )
        ...
        abstract_code = abstract_code_with_config(
            ...
            position_map,
            abstract_type_map,
            CodeAbstracterBase.DEFAULT_ABSTRACT_CONFIG,
        )
        ...
        symbol_table = {
            ...
            "abstract_table": abstract_type_map,
        }
...
def abstract_code_with_config(
    code: str,
    position_map: dict[str, list[tuple]],
    abstract_table: dict[str, dict],
    ...
    def enable_this_type_abstract(abstract_type: str) -> bool:
        ...
        if abstract_type in abstract_config and abstract_config[abstract_type]:
            return True
        return False
    ...
    for line, line_of_code in enumerate(code.splitlines(keepends=True)):
        ...
        sorted_items_in_line = sorted(
            items_in_line, key=lambda x: x[0]
        )  # sort by column
        ...
        for item_idx, item in enumerate(sorted_items_in_line):
            ...
            if not enable_this_type_abstract(abstract_type):
                continue
            add_sentinel_line += (
                line_of_code[last_col:this_item_start_col]
                + "$$$$"
                + line_of_code[
