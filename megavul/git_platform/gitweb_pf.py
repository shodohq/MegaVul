...
from megavul.util.utils import (
    ...
    get_unix_time_from_git_date_gitweb,
)
...
class GitWebPlatformBase(GitPlatformBase):
    ...
    def get_raw_commit_info(
        ...
        commit_date = get_unix_time_from_git_date_gitweb(
            object_headers[1].find("span", class_="datetime").text
        )

        # remove tree_hash and commit_hash
        tree_url_prefix, tree_url_postfix = (
            object_headers[5].find("td", class_="link").a["href"].split("?")
        )
        ...
        commit_msg = unicodedata.normalize(
            "NFKD", gitweb_page.find("div", class_="page_body").text
        )

        repo_name = ""
        for idx, a in enumerate(
            gitweb_page.find("div", class_="page_header").find_all("a")[2:]
        ):
            if idx > 0:
                repo_name += "/"
            repo_name += a.text
