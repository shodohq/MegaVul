...
from megavul.util.utils import (
    ...
    get_unix_time_from_git_date_cgit,
)
...
class CGitPlatformBase(GitPlatformBase):
    ...
    def get_raw_commit_info(
        ...
        while (commit_info is None) and (
            cgit_page.find("div", class_="content") is not None
        ):
        ...
        commit_date = get_unix_time_from_git_date_cgit(
        ...
        while (
            diff_table is None and diff_page.find("div", class_="content") is not None
        ):
    ...
    def can_handle_this_url(
        ...
        return (
            url_netloc
            in ["git.kernel.org", "cgit.freedesktop.org", "git.savannah.gnu.org"]
            and "git.savannah.gnu.org/gitweb" not in url
        )
