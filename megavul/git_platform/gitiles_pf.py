...
from megavul.util.utils import (
    gitiles_safe_get_bs4_request,
    gitiles_safe_get_request,
...
class GitilesPlatformBase(GitPlatformBase):
    ...
    def get_raw_commit_info(
        ...
        commit_date = get_unix_time_from_git_date_gitiles(
            commit_table[1].find_all("td")[-1].text
        )
        ...
        commit_msg = gitiles_page.find(
            "pre", class_="u-pre u-monospace MetadataMessage"
        ).text
        ...
        for d in diff_tree:
            if (
                d.find("span", class_="DiffTree-action DiffTree-action--add")
                is not None
            ):
                # this file is added , skip this file.
                continue
            if (
                d.find("span", class_="DiffTree-action DiffTree-action--delete")
                is not None
            ):
                # this file is deleted , skip this file
                continue
            if (
                d.find("span", class_="DiffTree-action DiffTree-action--rename")
                is not None
            ):
    ...
    def download_commit_with_save_dir(
        ...
        for f_path in need_download_file_paths:
            ...
            if download_content is None:
                logger.debug(
                    self.fmt_msg(
                        f"{raw_commit_info.repo_name}:{tree_hash} {f_path} {download_url} download error or file missing"
                    )
                )
                continue
...
if __name__ == "__main__":
    ...
    raw_commit_info = pf.get_raw_commit_info(
        global_logger,
