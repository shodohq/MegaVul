...
class GitPlatformBase(metaclass=ABCMeta):
    ...
    def resolve_raw_commit_and_download(
        self, logger: logging.Logger, raw_commit_info: RawCommitInfo
    ) -> Optional[DownloadedCommitInfo]:
        ...
        if len(set(a_dl_files)) != len(set(b_dl_files)):
            logger.debug(
                self.fmt_msg(
                    f"{raw_commit_info.repo_name}:{raw_commit_info.commit_hash}"
        ...
        if len(diff_file_paths) == 0:
            return None
        return DownloadedCommitInfo(
            raw_commit_info.repo_name,
            raw_commit_info.commit_msg,
            raw_commit_info.commit_hash,
            raw_commit_info.parent_commit_hash,
            raw_commit_info.commit_date,
            diff_file_paths,
            raw_commit_info.git_url,
        )
    ...
    def download_commit(
        ...
        return self.download_commit_with_save_dir(
            logger,
            raw_commit_info,
            need_download_file_paths,
            download_parent_commit,
            save_dir,
        )
