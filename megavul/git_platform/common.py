...
class CommitFile:
    ...
    def __repr__(self):
        ...
        if len(self.non_vulnerable_functions) > 5:
            non_vul_names = ", ".join(
                [func.func_name for func in self.non_vulnerable_functions[:5]]
            )
            non_vul_names += f", ...[Total {len(self.non_vulnerable_functions)} Non-Vulnerable Functions]"
        else:
            non_vul_names = ", ".join(
                [func.func_name for func in self.non_vulnerable_functions]
            )
...
class CommitInfo:
    ...
    raw_file_paths: list[
        str
    ]  # if the number of functions in file is empty, this file will be filtered
...
# ADD_MORE_LANGUAGE_NOTE: 対応言語を増やすには拡張子リストを追加する (例: GoFileExtension = ["go"])
...
def select_file_extension():
    ...
    # ADD_MORE_LANGUAGE_NOTE: 対応言語を増やすには elif ブランチを追加して AcceptedFileExtension を設定する
        ...
        raise RuntimeError(
            f"The extension of the {crawling_language} language is unknown"
        )
