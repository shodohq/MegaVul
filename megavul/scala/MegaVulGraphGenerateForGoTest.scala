...
class MegaVulGraphGenerateForGoTest extends GoCodeToCpgSuite {

  def importCode(inputFilePath: String): Cpg = {
    val content = Files.readString(Paths.get(inputFilePath))
    if (inputFilePath.endsWith(".go")) {
      // Go requires a package declaration; wrap the extracted function in package main
      return new DefaultTestCpgWithGo(".go").moreCode("package main\n" + content)
    }
    else
      throw new Exception(s"${inputFilePath} suffix can't recognized, only support .go suffix")
  }
  ...
  def generateGraphForFunction(function_dir_path: Path): Unit = {
    ...
    val func_path = function_dir_path.resolve(s"${file_name}.go")
    ...
    try {
      ...
      case e: NullPointerException => {
        println(s"NPE Exception found!! for ${func_path.toString}")
      }
  }
  ...
  "generateGraph" should {
    ...
    println("MegaVul Generate Go Graph Done!")
  }
