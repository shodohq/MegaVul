...
class TestGnomeTokenLoading:
    ...
    def test_returns_stripped_token_when_file_exists(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            StorageLocation, "gitlab_gnome_token_path", staticmethod(lambda: token_file)
        )
        from megavul.git_platform import gitlab_pf
...
class TestGitLabGnomeUrlParsing:
    ...
    def test_multi_level_namespace(self):
        result = self.parse(
            "https://gitlab.gnome.org/ns/sub/project/-/merge_requests/5"
        )
...
class TestFindCommitsFromMRViaV4Api:
    ...
    def test_normal_case_returns_commit_urls(self):
        commits = [
            self._make_commit(f"https://gitlab.gnome.org/proj/-/commit/{i}")
            for i in range(3)
        ]
...
class TestFindCommitsFromIssueViaV4Api:
    ...
    def test_closed_via_commit_returns_url(self):
        notes = [
            self._make_note(
                "closed via commit abc123def456abc123def456abc123def456abc1"
            )
        ]
    ...
    def test_closed_via_mr_delegates_to_mr_func(self):
        ...
        with patch(
            "megavul.git_platform.gitlab_pf.find_commits_from_mr_via_v4_api",
    ...
    def test_exceeds_threshold_returns_empty(self):
        ...
        notes = [
            self._make_note(f"closed via commit {'a' * 40}")
...
class TestFindCommitsFromGitlabRouting:
    ...
    def test_gnome_mr_url_uses_v4_api(self):
        ...
        with (
            ...
            patch(
                "megavul.git_platform.gitlab_pf.load_gitlab_gnome_token",
                return_value=None,
            ),
        ):
            result = self.func(
    ...
    def test_gnome_issue_url_uses_v4_api(self):
        ...
        with (
            patch(
                "megavul.git_platform.gitlab_pf.find_commits_from_issue_via_v4_api",
                return_value=expected,
            ) as mock_issue,
            patch(
                "megavul.git_platform.gitlab_pf.load_gitlab_gnome_token",
    ...
    def test_gitlab_com_uses_legacy_path(self):
        with patch(
            "megavul.git_platform.gitlab_pf.find_commits_from_pr_in_gitlab",
            return_value=["https://gitlab.com/x/y/-/commit/abc"],
        ) as mock_legacy:
            _result = self.func("https://gitlab.com/gnutls/gnutls/merge_requests/657")
