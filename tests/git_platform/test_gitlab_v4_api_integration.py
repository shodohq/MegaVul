...
def pat_token() -> str:
    ...
    if token is None:
        pytest.skip(
            "megavul/gitlab_gnome_token.txt が存在しないため integration test をスキップ"
        )
    return token
...
def is_gnome_commit_url(url: str) -> bool:
    ...
    return bool(re.match(r"^https://gitlab\.gnome\.org/.+/-/commit/[0-9a-f]{40}$", url))
...
class TestFindCommitsFromGitlabE2E:
    ...
    def test_mr_url_with_fragment_is_handled(self, pat_token):
        url = (
            "https://gitlab.gnome.org/GNOME/gdk-pixbuf/-/merge_requests/121#note_99999"
        )
        result = find_commits_from_gitlab(url)
        assert isinstance(result, list)
